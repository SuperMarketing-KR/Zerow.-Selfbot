...
stream_url = "https://www.twitch.tv/katexod"
tts_language = "tts_language"
__author__ = "</KatΣxoD>#0002"
...
def Init():
    ...
    try:
        ...
        os.system(f'title </KatΣxoD>#0002 | Zerow. Selfbot - Version 1')
...
colorama.init()
Zerow = discord.Client()
Zerow = commands.Bot(description='Zerow. Selfbot', command_prefix=prefix, self_bot=True)
Zerow.msgsniper = True
Zerow.slotbot_sniper = True
Zerow.giveaway_sniper = True
Zerow.yui_kiss_user = None
Zerow.yui_kiss_channel = None
Zerow.yui_hug_user = None
Zerow.yui_hug_channel = None
Zerow.sniped_message_dict = {}
Zerow.sniped_edited_message_dict = {}
Zerow.copycat = None
Zerow.remove_command('help')
...
async def on_connect():
    ...
    ███████╗███████╗██║  ██║╚██████╔╝╚███╔███╔╝
    ╚══════╝╚══════╝╚═╝  ╚═╝ ╚═════╝  ╚══╝╚══╝ 
    {Fore.RED}Zerow. Selfbot v1 | Made by {Fore.RESET}{__author__}
    {Fore.MAGENTA}Logged In As: {Fore.RESET}{Zerow.user}
    {Fore.MAGENTA}Guilds: {Fore.RESET}{len(Zerow.guilds)}
    {Fore.RED}Prefix: {Fore.RESET}{Zerow.command_prefix}
    (can't change the prefix for the moment)

    - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    {Fore.BLUE}NitroSniper: {Fore.RESET}{nitro}{Fore.MAGENTA}
    {Fore.MAGENTA}Selfbot Ready{Fore.MAGENTA}
    ''' + Fore.RESET)
...
async def on_message(message):
    if Zerow.copycat is not None and Zerow.copycat.id == message.author.id:
    ...
    if f'Congratulations <@{Zerow.user.id}>' in message.content:
        if Zerow.giveaway_sniper:
            if message.author.id == 788470631968145411:
                print(""
    ...
    await Zerow.process_commands(message)

@Zerow.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(Zerow.voice_clients, guild=ctx.guild)
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Zerow.yui_kiss_user = user.id
        Zerow.yui_kiss_channel = ctx.channel.id
        if Zerow.yui_kiss_user is None or Zerow.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Zerow.yui_kiss_user is not None and Zerow.yui_kiss_channel is not None:
            await Zerow.get_channel(Zerow.yui_kiss_channel).send('yui kiss ' + str(Zerow.yui_kiss_user), delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Zerow.yui_hug_user = user.id
        Zerow.yui_hug_channel = ctx.channel.id
        if Zerow.yui_hug_user is None or Zerow.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Zerow.yui_hug_user is not None and Zerow.yui_hug_channel is not None:
            await Zerow.get_channel(Zerow.yui_hug_channel).send('yui hug ' + str(Zerow.yui_hug_user), delete_after=0.1)
            await asyncio.sleep(60)

@Zerow.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1 
            channel = Zerow.get_channel(int(channelid))
...
@Zerow.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    Zerow.slotbot_sniper = False
...
@Zerow.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    Zerow.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Zerow.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Zerow.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == Zerow.user.id:
        return
    if Zerow.msgsniper:
        if isinstance(message.channel, discord.DMChannel) or isinstance(message.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(Zerow.sniped_message_dict) > 1000:
        Zerow.sniped_message_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        channel_id = message.channel.id
        message_content = "`" + str(discord.utils.escape_markdown(str(message.author))) + "`: " + str(
            message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Zerow.sniped_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Zerow.event
async def on_message_edit(before, after):
    if before.author.id == Zerow.user.id:
        return
    if Zerow.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel) or isinstance(before.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(Zerow.sniped_edited_message_dict) > 1000:
        Zerow.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Zerow.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
async def snipe(ctx):
    ...
    if currentChannel in Zerow.sniped_message_dict:
        await ctx.send(Zerow.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")


@Zerow.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Zerow.sniped_edited_message_dict:
        await ctx.send(Zerow.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")
...
async def help(ctx):
  ...
  embed.set_footer(text="- 	𝐙𝐞𝐫𝐨𝐰.",icon_url="https://cdn.discordapp.com/attachments/794368127705415701/794601911206150164/Zerow.png")
  embed.set_image(url="https://cdn.discordapp.com/attachments/794368127705415701/794665291421712397/Zerow_self_banner.gif")
  embed.set_author(name="𝐙𝐞𝐫𝐨𝐰.  𝙁𝙊𝙍𝙀𝙑𝙀𝙍")
...
@Zerow.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if Zerow.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(Zerow.copycat))
    Zerow.copycat = None


@Zerow.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    Zerow.copycat = user
    await ctx.send("Now copying " + str(Zerow.copycat))


@Zerow.command(aliases=["9/11", "911", "terrorist"])
...
@Zerow.command(aliases=["spamchangegcname", "changegcname"])
...
@Zerow.command(aliases=["fakename"])
...
@Zerow.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
        ...
        async with cs.get(f"https://nekobot.xyz/api/imagegen?type=tweet&username={username}&text={message}") as r:
            ...
            try:
                ...
                with io.BytesIO(image) as file:
                    await ctx.send(file=discord.File(file, f"zerow_tweet.png"))
            except:
                await ctx.send(res['message'])


@Zerow.command(aliases=["distort"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"zerow_magik.png"))
        except:
            await ctx.send(res['message'])
    else:
...
@Zerow.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in Zerow.guilds:
        await guild.ack()
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"zerow_fry.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"zerow_blur.png"))
        except:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_supreme.png"))
    except:
        await ctx.send(endpoint)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_dark_supreme.png"))
    except:
        await ctx.send(endpoint)


@Zerow.command(aliases=["facts"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_facts.png"))
    except:
        await ctx.send(endpoint)


@Zerow.command(aliases=["blurp"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"zerow_blurpify.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"zerow_invert.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
...
@Zerow.command(aliases=["pornhublogo", "phlogo"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_pornhub_logo.png"))
    except:
        await ctx.send(endpoint)


@Zerow.command(aliases=["pornhubcomment", 'phc'])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["message"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_pornhub_comment.png"))
    except:
        await ctx.send(res["message"])
...
@Zerow.command(aliases=["reversesearch", "anticatfish", "catfish"])
...
@Zerow.command(aliases=['pfp', 'avatar'])
...
@Zerow.command(aliases=["del", "quickdel"])
...
@Zerow.command(name='1337speak', aliases=['leetspeak'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
            try:
                await Zerow.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
            except discord.HTTPException as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Zerow.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await Zerow.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Zerow.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@Zerow.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Zerow.command(aliases=['dong', 'penis'])
...
@Zerow.command(aliases=['changehypesquad'])
...
@Zerow.command(aliases=['fakeconnection', 'spoofconnection', 'spoofcon', "fakecon"])
async def fakenet(ctx, _type=None, *, name=None):
    ...
    if r.status_code == 200:
        ...
        await ctx.send(
            '**[ERROR]** `zerow Fake-Connection doesn\'t work anymore because Discord patched connection-spoofing`')
...
@Zerow.command(aliases=["copyguild", "servercopy"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await Zerow.create_guild(f'backup-{ctx.guild.name}')
...
async def fear(ctx):
    ...
    try:
        await ctx.guild.edit(
            name="zerow On Top",
            description="zerow Runs You",
            reason="zerow Hoed You",
            icon=None,
            banner=None
        )
    except:
        pass
    for _i in range(125):
        await ctx.guild.create_text_channel(name="Fear zerow")
    for _i in range(125):
      await ctx.guild.create_voice_channel(name="No Mercy zerow")    
    for _i in range(250):
        await ctx.guild.create_role(name="zerow Owns You", color=RandomColor())


@Zerow.command(aliases=["banwave", "banall", "etb"])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="zerow")
        except:
            pass
...
@Zerow.command(aliases=["kickall", "kickwave"])
async def masskick(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason="zerow")
        except:
            pass


@Zerow.command(aliases=["spamroles"])
async def massrole(ctx):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="zerow", color=RandomColor())
        except:
            return


@Zerow.command(aliases=["masschannels", "masschannel", "spamchannels"])
async def ctc(ctx):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="zerow Owns You")
        except:
            return

@Zerow.command(aliases=["massvoicechannels", "massvoicechannel"])
async def cvc(ctx):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_voice_channel(name="zerow Runs You")
        except:
            return

@Zerow.command(aliases=["delchannel"])
...
@Zerow.command(aliases=["deleteroles"])
...
@Zerow.command(aliases=["unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = Zerow.get_user(user.id)
    if ctx.author.id == Zerow.user.id:
        return
    else:
        try:
            await user.send(message)
...
@Zerow.command(name='get-color', aliases=['color', 'colour', 'sc', "hexcolor", "rgb"])
...
@Zerow.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@Zerow.command(aliases=['serverbanner'])
...
@Zerow.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@Zerow.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    Zerow.command_prefix = str(prefix)
...
@Zerow.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@Zerow.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in Zerow.user.relationships:
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(Zerow.user.relationships)
    for relationship in Zerow.user.relationships:
...
@Zerow.command(aliases=["changeregions", "changeregion", "regionschange"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_dog.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_cat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_sadcat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_bird.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_fox.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_anal.gif"))
    except:
        em = discord.Embed()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_erofeet.png"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_feet.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_hentai.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_boobs.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_tits.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_blowjob.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_neko.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_lesbian.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_cumslut.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_pussy.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"zerow_waifu.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"zerow_feed.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"zerow_tickle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"zerow_slap.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"zerow_hug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"zerow_cuddle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"zerow_smug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"zerow_pat.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"zerow_kiss.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
async def purge(ctx, amount: int):
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Zerow.user).map(
            lambda m: m):
        try:
            await message.delete()
        except:
...
@Zerow.command(aliases=["streaming"])
...
@Zerow.command(aliases=["stopstreaming", "stopstatus", "stoplistening", "stopplaying", "stopwatching"])
async def stopactivity(ctx):
    await ctx.message.delete()
    await Zerow.change_presence(activity=None, status=discord.Status.dnd)
...
@Zerow.command(aliases=["fliptable"])
...
@Zerow.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await Zerow.logout()


@Zerow.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
